...
Workspace allowance depends on the plan: Paid Ads Autopilot includes 3, every other plan includes 1, and Paid Ads Agency ($279/month) is uncapped. Extra SEO workspaces are add-ons — $129/month on SEO Autopilot, $599/month on Traffic Printer.
...
White label is part of the Agency plan: your logo, colors, app domain, and email sender instead of Ryze's. It is set once for the organization and applies to every client workspace. See [Branding](/white-label/branding), [Domains](/white-label/domains), and [Client access](/white-label/access).
...
Billing sits at the organization level, above workspaces. Invoices cover the organization rather than being issued per client, so agencies rebilling clients typically allocate internally from a single invoice. See [Billing FAQ](/billing/faq).
...
Paid Ads Agency ($279/month) removes the workspace cap. Requirements above that are handled case by case. Contact [support@get-ryze.ai](mailto:support@get-ryze.ai) with the number of clients and workspaces you expect to run.
