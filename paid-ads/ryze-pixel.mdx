...
<Steps>
  ...
  <Step title="Paste it before </head> on every page">
    ...
    ```
    On most site builders this goes into the global head or custom code section. On Shopify, add it to `theme.liquid` right before `</head>`.
  </Step>
...
Use `order_id` on every purchase. Ryze deduplicates by it, so a purchase that fires both in the browser and from your backend counts once. Rotate the secret from the same dialog if it ever leaks; the old key stops working immediately.
