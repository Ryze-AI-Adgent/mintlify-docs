...
<Steps>
  ...
  <Step title="Save the article">
    Parse the JSON body and store the article so your site renders it at a stable URL — ideally `your blog URL + slug` (see below). Upsert by `slug` and respond `200`.
  </Step>
</Steps>
...
- **Images**: the payload carries the article HTML only. Featured images are not included.
